...
- TCP를 세그먼트 단위로 자르고 세그먼트를 IP 패킷(데이터그램)에 담아 전달
  ...
  - TCP 세그먼트 헤더(20바이트)
    - 발신지 목적지 포트번호, TCP 제어 플래그, SYN 순서번호, ACK 확인응답 값
...
- 지연 원인
  - URI로 부터 웹 서버의 IP와 Port번호를 알아내는 과정
  - TCP 커넥션 요청
  - HTTP 요청 전송, 요청메시지 처리
  - HTTP 응답
...
- 동시에, 한번에, 빠르게 받을 수 있지만 클라이언트의 네트워크 대역폭이 좁다면 제한된 대역폭 내에서 병렬 커넥션의 성능상의 장점은 거의 없어진다. 
...
- HTTP/1.1은 Keep-alive 커넥션을 지원하지 않는 대신 **기본으로 지속 커넥션을 활성화했다**
