...
메모리에 적재된 클래스(바이트 코드)들을 기계어로 변경하여 명령어단위로 실행한다.  
바이트코드를 운영체제에 맞게 해석해주는 역할을 수행하며 클래스 파일을 실행시키는 방법에 따라 두 가지로 분류된다.  

### 인터프리터

각 플랫폼에 맞는 인터프리터가 바이트 코드를 실행하기 때문에 자바의 특징인 `Write Once Run Anywhere`이 가능한 것이다.  
인터프리터는 바이트 코드를 읽고 운영체제가 실행할 수 있도록 기계어(native code)로 변경하는 역할을 수행한다.  

JVM 인터프리터는 런타임에 바이트코드를 **한 라인씩** 읽고 실행한다. 이때 속도 문제가 발생하게 되는데, 미리 컴파일을 통해 기계어로 변경되는 언어에 비해 속도가 느려진다. 코드 각 줄을 매번 읽고 번역해야한다.
...
코드 전체에서 반복되어 호출되는 메소드는 JIT 컴파일러에 의해 기계어로 변환되어 Native Method Stack에 저장된다.    
...
컴파일 임계치가 일정 횟수에 도달한 코드는 컴파일하기에 충분한 정보가 쌓였다고 판단되어진다.
